            ...
            if len(parts) >= 2:
                try:
                    ...
                    if tax_id != parent_tax_id:
                        edges.append({"id1": tax_id, "id2": parent_tax_id, "weight": 1.0})
...
def ensure_taxdump(data_dir: Path) -> tuple[Path, Path, Path | None]:
    ...
    with (
        urllib.request.urlopen(TAXDUMP_URL) as response,
        archive_path.open("wb") as out_f,
    ):
...
def download_taxonomy(data_dir: Path | None = None):
    """Download and parse NCBI taxonomy data.

    Args:
        data_dir: Directory for data files (defaults to ./data)

    Returns:
        Path to the output edgelist file
    """
    if data_dir is None:
