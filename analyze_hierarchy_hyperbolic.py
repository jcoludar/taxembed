...
def load_embeddings(ckpt_path):
    ...
    if norms.max() >= 1.0:
        print(f"  ⚠️  WARNING: Some embeddings are outside the Poincaré ball (norm >= 1.0)!")
...
def analyze_depth_vs_norm(emb, idx2tax, taxonomy):
    ...
    ax1.set_title(f'Depth vs. Radial Position\nPearson r = {pearson_r:.3f}', 
...
def main():
    ...
    for rank in ["phylum", "class", "order"]:
        try:
            sep, qual = analyze_hierarchical_clustering_hyperbolic(emb, idx2tax, taxonomy, rank=rank)
